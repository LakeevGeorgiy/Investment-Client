...
MenuWindow::MenuWindow(QWidget *root) : 
    ...
    , QWidget(root)
{
    ...
    connect(login_button, &QPushButton::clicked, this, &MenuWindow::ClickLogIntButton);
    menu_layout_left->addWidget(login_button);

    auto register_button = new QPushButton("Register");
    connect(register_button, &QPushButton::clicked, this, &MenuWindow::ClickRegistrationButton);
    menu_layout_left->addWidget(register_button);

    user_stocks_button_->setVisible(false);
    connect(user_stocks_button_, &QPushButton::clicked, this, &MenuWindow::UserStocksRequest);

    auto exchange_button = new QPushButton("Exchange");
    connect(exchange_button, &QPushButton::clicked, this, &MenuWindow::AllStocksRequest);

    // connect(this, &MenuWindow::SendUserStockRequest, user_stocks_request_.get(), &Request::GetRequest);
    connect(user_stocks_request_.get(), &Request::gotHttpData, this, &MenuWindow::UserStocksRead);

    // connect(this, &MenuWindow::SendAllStocksRequest, all_stocks_request_.get(), &Request::GetRequest);
    connect(all_stocks_request_.get(), &Request::gotHttpData, this, &MenuWindow::AllStocksRead);
...
void MenuWindow::UserStocksRequest()
{
...
void MenuWindow::AllStocksRequest(){
...
void MenuWindow::UserStocksRead(int status_code, QByteArray data) {
...
void MenuWindow::AllStocksRead(int status_code, QByteArray data){
...
void MenuWindow::PrintStocks(QJsonArray &stocks_json, const QString& button_text, std::function<void(Stock)>& button_click){
...
void MenuWindow::ClickLogIntButton()
{
    // stacked_widget_->setCurrentIndex(2);
}

void MenuWindow::ClickRegistrationButton(){
